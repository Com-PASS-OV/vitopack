...
Tento dokument tě provede:
1. instalací nástrojů,
2. vygenerováním dokumentace a buildu,
3. spuštěním testů a `R CMD check`,
4. publikací na GitHub a nasazením pkgdown stránky,
5. odesláním balíčku na CRAN.
...
V RStudiu: **File → New Project → Existing Directory →** `Desktop/vitopack/vitopack`
...
Tím se z `roxygen2` komentářů v `R/*.R` vytvoří soubory v `man/` a doplní/aktualizuje `NAMESPACE`.
...
1. Vytvoř repo na GitHubu (např. `donvito/vitopack`). Pokud používáš jiný název nebo organizaci, **uprav `URL` a `BugReports` v souboru `DESCRIPTION` a `_pkgdown.yml`** — momentálně tam je `donvito/vitopack`.
...
3. Spusť:
   ...
   <https://xmpalantir.wu.ac.at/cransubmit/> a pošle e-mail z tvé adresy
   `ondrej.vit@com-pass.cz` k potvrzení.
